 ...
 *     description: Prüft, ob der Server läuft und die Datenbank erreichbar ist.
 *     tags: [System]
 *     responses:
 *       200:
 *         description: OK, System läuft
 *         content:
 *           application/json:
 *             schema:
 *               type: object
    ...
    async (req, res) => {
        ...
        try {
            ...
            if (discount_type !== undefined) {
                updates.push(`discount_type = $${paramIdx++}`);
            ...
            if (min_order_amount !== undefined) {
                updates.push(`min_order_amount = $${paramIdx++}`);
                values.push(min_order_amount);
            }
            if (max_discount_amount !== undefined) {
                updates.push(`max_discount_amount = $${paramIdx++}`);
                values.push(max_discount_amount);
            }
            if (usage_limit !== undefined) {
                updates.push(`usage_limit = $${paramIdx++}`);
                values.push(usage_limit);
            }
            if (customer_usage_limit !== undefined) {
                updates.push(`customer_usage_limit = $${paramIdx++}`);
                values.push(customer_usage_limit);
            }
            if (valid_from !== undefined) {
                updates.push(`valid_from = $${paramIdx++}`);
                values.push(valid_from);
            }
            if (valid_until !== undefined) {
                updates.push(`valid_until = $${paramIdx++}`);
            ...
            if (applies_to !== undefined) {
                updates.push(`applies_to = $${paramIdx++}`);
                values.push(applies_to);
            }
            if (product_ids !== undefined) {
                updates.push(`product_ids = $${paramIdx++}`);
                values.push(product_ids);
            }
            if (category_ids !== undefined) {
                updates.push(`category_ids = $${paramIdx++}`);
                values.push(category_ids);
            }
            if (exclude_sale_items !== undefined) {
                updates.push(`exclude_sale_items = $${paramIdx++}`);
                values.push(exclude_sale_items);
            }
            if (excluded_product_ids !== undefined) {
                updates.push(`excluded_product_ids = $${paramIdx++}`);
                values.push(excluded_product_ids);
            }
            if (excluded_category_ids !== undefined) {
                updates.push(`excluded_category_ids = $${paramIdx++}`);
                values.push(excluded_category_ids);
            }
            if (first_order_only !== undefined) {
                updates.push(`first_order_only = $${paramIdx++}`);
                values.push(first_order_only);
            }
            if (requires_minimum_items !== undefined) {
                updates.push(`requires_minimum_items = $${paramIdx++}`);
                values.push(requires_minimum_items);
            }
        ...
        if (coupon.customer_usage_limit && customer_email) {
            ...
            if (parseInt(customerUsage[0].count) >= coupon.customer_usage_limit) {
                return res.status(400).json({
                    error: `You can only use this coupon ${coupon.customer_usage_limit} time(s)`
                });
            }
        }
...
app.post('/api/owner/shops', authMiddleware, requireRole('owner', 'admin'), async (req, res) => {
    ...
    if (!/^[a-z0-9-]+$/.test(slug)) {
        return res
            .status(400)
            .json({ error: 'Slug can only contain lowercase letters, numbers, and hyphens' });
    }
...
// ENTSCHÄRFT: Nur noch sehr offensichtliche Spam-Begriffe blockieren
const BLOCKED_PRODUCT_NAMES = ['auto-generated', 'placeholder produkt', 'test-product-123'];
const MAX_PRODUCTS_PER_MINUTE = 10; // Erhöht von 5
...
function isAutoGeneratedProduct(name, sku) {
    ...
    // Nur exakte Übereinstimmungen blockieren
    return BLOCKED_PRODUCT_NAMES.some((blocked) => lowerName === blocked);
}
...
app.post('/api/owner/products', authMiddleware, requireRole('owner', 'admin'), async (req, res) => {
    ...
    if (isAutoGeneratedProduct(req.body.name, req.body.sku)) {
        console.warn(
            `🚫 BLOCKED auto-generated product attempt by user ${req.user.id}: ${req.body.name}`
        );
    ...
    try {
        ...
        const pgImageUrls = image_urls
            ? `{${image_urls.map((url) => `"${url.replace(/"/g, '"')}"`).join(',')}}`
            : null;
...
// Update inventory
app.put(
    '/api/owner/products/:id/inventory',
    authMiddleware,
    requireRole('owner', 'admin'),
    async (req, res) => {
...
// Set primary image
app.put(
    '/api/owner/images/:imageId/primary',
    authMiddleware,
    requireRole('owner', 'admin'),
    async (req, res) => {
        try {
        ...
        `,
            ...
            await pool.query('UPDATE product_images SET is_primary = false WHERE product_id = $1', [
                productId
            ]);

            // Set this as primary
            await pool.query('UPDATE product_images SET is_primary = true WHERE id = $1', [
                req.params.imageId
            ]);
...
app.post('/api/orders', async (req, res) => {
    ...
    try {
        ...
        if (session_id) {
            await client.query('DELETE FROM carts WHERE session_id = $1 AND shop_id = $2', [
                session_id,
                shop_id
            ]);
        }
        ...
        try {
            // Get shop details
            const { rows: shopRows } = await pool.query('SELECT * FROM shops WHERE id = $1', [
                shop_id
            ]);
            const shop = shopRows[0];

            // Get owner email
            const { rows: ownerRows } = await pool.query('SELECT email FROM users WHERE id = $1', [
                shop.owner_id
            ]);
...
app.post('/api/orders/:orderId/payment/confirm', async (req, res) => {
    ...
    `,
        ...
        const shop = rows[0];
        const stripeClient =
            getStripeClient(shop) || require('stripe')(process.env.STRIPE_SECRET_KEY);
...
app.get(
    '/api/owner/orders/:orderId/invoice',
    authMiddleware,
    requireRole('owner', 'admin'),
    async (req, res) => {
        try {
        ...
        `,
            ...
            res.setHeader(
                'Content-Disposition',
                `inline; filename="Rechnung-${order.order_number}.pdf"`
            );
            res.send(pdfBuffer);
        } catch (error) {
...
app.post('/api/shipping/calculate', async (req, res) => {
    ...
    try {
        ...
        const methodsWithPrice = methods.map((m) => {
            ...
            if (
                req.shop.free_shipping_threshold &&
                cart_total >= req.shop.free_shipping_threshold
            ) {
                finalPrice = 0;
            }
...
app.post(
    ...
    async (req, res) => {
        ...
        try {
            ...
            if (is_default) {
                await pool.query('UPDATE tax_rates SET is_default = false WHERE shop_id = $1', [
                    shopId
                ]);
            }
...
function getPaymentInstructions(method, order) {
    const baseInstructions = {
        ...
        paypal_friends:
            'Bitte sende den Betrag per PayPal an: [E-MAIL]\n\nWichtig: Als "Freunde & Familie" senden!'
    };
